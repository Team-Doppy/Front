...
class _RecapContentScreenState extends State<RecapContentScreen> {
  ...
  void initState() {
    ...
    debugPrint(
      '[RecapContentScreen] 파싱할 JSON keys: ${jsonToParse.keys.toList()}',
    );
    debugPrint('[RecapContentScreen] hero: ${jsonToParse['hero']}');
    debugPrint('[RecapContentScreen] blocks: ${jsonToParse['blocks']}');
    debugPrint(
      '[RecapContentScreen] blocks 개수: ${(jsonToParse['blocks'] as List?)?.length ?? 0}',
    );

    _doc = RecapDoc.fromJson(jsonToParse);

    debugPrint('[RecapContentScreen] 파싱 완료 - hero.title: ${_doc.hero.title}');
    debugPrint('[RecapContentScreen] 파싱 완료 - blocks 개수: ${_doc.blocks.length}');
