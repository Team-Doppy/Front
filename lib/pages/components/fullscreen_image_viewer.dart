...
class _FullscreenImageViewerState extends State<FullscreenImageViewer>
  ...
  Future<void> _toggleCommentLike(MediaComment comment) async {
    try {
      ...
      // 낙관적 업데이트
      final oldEmotionCounts = Map<String, int>.from(comment.emotionCounts);
      final oldMyEmotions = List<String>.from(comment.myEmotions);

      final newMyEmotions =
          comment.isLiked
              ? oldMyEmotions.where((e) => e != emoji).toList()
              : [...oldMyEmotions, emoji];

      final newEmotionCounts = Map<String, int>.from(oldEmotionCounts);
      if (comment.isLiked) {
      ...
      setState(() {
        ...
        if (index != -1) {
          ...
          _commentsByImage[_currentImageUrl] = List.from(currentComments);
        }
      });
  ...
  Future<void> _submitComment() async {
    ...
    if (mediaId == null || mediaId.isEmpty) {
      ErrorHandler.showError(context, '메시지 전송 중 오류가 발생했어요');
      return;
    }
          ...
          children: [
            ...
            Builder(
              builder: (context) {
                ...
                final adjustedSheetTop = sheetTop - keyboardHeight;

                return Positioned(
                  top: adjustedSheetTop,
                  left: 0,
                  right: 0,
                  bottom: 0,
                        ...
                        children: [
                          ...
                          if (widget.postAuthorProfileUrl != null)
                            CircleAvatar(
                              ...
                              backgroundImage:
                                  widget.postAuthorProfileUrl!.isNotEmpty
                                      ? NetworkImage(
                                        widget.postAuthorProfileUrl!,
                                      )
                                      : null,
                              child:
                                  widget.postAuthorProfileUrl!.isEmpty
                                      ? const Icon(
                                        Icons.person,
                                        size: 20,
                                        color: Colors.white,
                          ...
                          GestureDetector(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  '${widget.commentCount ?? _imageComments.length}',
                                  style: const TextStyle(
                                    color: Colors.white,
                                    fontSize: 15,
                                    fontWeight: FontWeight.w600,
