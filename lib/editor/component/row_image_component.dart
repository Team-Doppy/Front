...
class _ImageRowComponentState extends State<ImageRowComponent>
  ...
  // 🎯 보기 모드: 탭 위치 저장 (자세히보기에서 어떤 이미지 열지 판별)
  ...
  Key _rowImageKey(int index, String imageUrl) {
    ...
    final dupCount = widget.imageUrls
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final missingCount = widget.imageUrls
          .where(
            (url) =>
                !_imageSizes.containsKey(url) && !_measuringUrls.contains(url),
          )
          .length;
      if (missingCount > 0) {
        debugPrint(
  ...
  Widget build(BuildContext context) {
    ...
    // 🎯 보기 모드에서 스포일러/자세히보기 분기 위해 선계산
    bool isRowSpoiler = false;
    if (!widget.isEditing) {
      try {
        final node = doc?.getNodeById(widget.nodeId);
        Map<String, dynamic>? meta;
        if (node is ImageRowNode) meta = node.metadata;
        isRowSpoiler = context.select<NodeComponentService, bool>(
          (s) => s.shouldShowImageSpoiler(widget.nodeId, meta),
        );
      } catch (_) {}
    }

    // 🎯 네트워크 모드 + 업로드 중: 드래그(스왑/레이아웃 변경) 철저히 차단
              ...
              GestureDetector(
                ...
                onTap: widget.isEditing && widget.dragService != null
                    ...
                    : (!widget.isEditing
                        ? () {
                            ...
                            if (isRowSpoiler) {
                              ...
                              if (node is ImageRowNode &&
                                ...
                                initialIndex = _findClickedImageIndex(
                                      node,
                        ...
                        : null),
                onLongPressStart: widget.isEditing &&
                        widget.dragService != null &&
                        !isUploading
                    ? (details) {
                ...
                onLongPressMoveUpdate:
                    ...
                    : null,
                onLongPressEnd: widget.isEditing &&
                        widget.dragService != null &&
                        !isUploading
                    ? (_) {
  ...
  Widget _buildRowImageWidget(int index, String imageUrl, double maxWidth) {
    ...
    if (!isLocal) {
      _lastNetworkProviders[imageUrl] = baseProvider;
    }
