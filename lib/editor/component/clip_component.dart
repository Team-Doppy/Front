...
Future<void> pauseAllVideoPlayers({
  ...
  for (final proxy in proxies) {
    // 🎯 실제 컨트롤러가 있으면 직접 제어 (프록시 콜백은 중복 호출 방지)
    final c = proxy.controller;
    if (c != null) {
      try {
        // 🎯 dispose된 컨트롤러 체크: value 접근 시 예외 발생 가능
        ...
        // dispose된 컨트롤러 접근 시 예외 발생 가능
        debugPrint('[ClipComponent] 프록시 컨트롤러 일시정지 실패: $e');
      }
    } else {
      // 🎯 실제 컨트롤러가 없으면 프록시 콜백 사용 (초기화 전 상태)
      try {
        proxy.pause?.call();
      } catch (e) {
        debugPrint('[ClipComponent] 프록시 pause 콜백 실패: $e');
      }
    }
  }
  ...
  for (final c in externals) {
    try {
      ...
      // dispose된 컨트롤러 접근 시 예외 발생 가능
      debugPrint('[ClipComponent] 외부 컨트롤러 일시정지 실패: $e');
      // 🎯 dispose된 컨트롤러는 Set에서 제거
      _externalVideoControllers.remove(c);
    }
  }
  ...
  for (final proxy in proxies) {
    ...
    if (c != null) {
      ...
      try {
        ...
        debugPrint('[ClipComponent] 컨트롤러 dispose 실패: $e');
      }
      // 🎯 프록시에서 컨트롤러 참조 제거
    ...
    // 🎯 프록시 콜백도 정리
    proxy.pause = null;
    proxy.toggleMute = null;
    proxy.restartVideo = null;
    proxy.hasPlayedOnce = null;
  }

  // 3) 프록시 맵 clear
  videoPlayerControllers.clear();

  // 4) 외부 컨트롤러들 dispose 및 정리
  final externals = _externalVideoControllers.toList(growable: false);
  for (final c in externals) {
    try {
      if (c.value.isInitialized) {
      ...
      debugPrint('[ClipComponent] 외부 컨트롤러 dispose 실패: $e');
    }
  }
...
Future<void> muteAllVideos() async {
  ...
  for (final entry in videoPlayerControllers.entries) {
    ...
    try {
      ...
      debugPrint('[ClipComponent] 컨트롤러 음소거 실패: $e');
    }
  }
  ...
  for (final c in externals) {
    try {
      ...
      debugPrint('[ClipComponent] 외부 컨트롤러 음소거 실패: $e');
      _externalVideoControllers.remove(c);
    }
  }
...
class _ClipComponentState extends State<_ClipComponent> with DocumentComponent {
    ...
    try {
      ...
      if (node is ClipNode) {
        ...
        if (aspectRatioValue != null) {
          _metadataAspectRatio = (aspectRatioValue is num)
  ...
  Widget build(BuildContext context) {
    ...
    final DocumentSelection? composerSelection = widget.isEditing
    ...
    if (widget.isEditing) {
      try {
        isUploading = context.select<IUploadService, bool>(
  ...
  /// 압축(FFmpeg)/처리 중인지 확인 (편집 모드에서만)
  ///
  /// UploadService의 _processingRefIds를 통해 압축 상태를 확인합니다.
  /// VideoUploadUtils에서 markRefAsProcessing/markRefAsProcessed를 호출하면
  /// 이 메서드가 정확한 상태를 반환합니다.
  bool _isCompressing(BuildContext context) {
    if (!widget.isEditing) return false;
    try {
      final service = context.select<IUploadService, bool>(
        (uploadService) =>
            uploadService.hasActiveCompressionForRef(widget.nodeId),
      );
      return service;
    } catch (e) {
      debugPrint('[ClipComponent] UploadService 확인 실패: $e');
...
class _VideoPlayerWidgetState extends State<_VideoPlayerWidget> {
  ...
  IUploadService? _uploadService; // 🎯 업로드 서비스 참조
  ...
  void initState() {
    ...
    if (widget.isEditing) {
      try {
        final service = Provider.of<UploadService>(context, listen: false);
  ...
  void didUpdateWidget(_VideoPlayerWidget oldWidget) {
    ...
    final oldKey = oldWidget.localPath.isNotEmpty
        ? oldWidget.localPath
        : oldWidget.url;
  ...
  Future<void> _initializeVideo() async {
    ...
    try {
      ...
      if (mounted && _controller != null && !_didRunPostInitSetup) {
        ...
        // 썸네일 생성 (편집 모드에서만)
        final cacheKey = widget.localPath.isNotEmpty
            ? widget.localPath
            : widget.url;
        if (mounted &&
            widget.isEditing &&
  ...
  ///
  /// 주의: 썸네일 생성은 외부 콜백을 통해 처리되어야 합니다.
  /// VideoUploadUtils.generateThumbnail을 사용하거나, 외부에서 콜백을 제공해야 합니다.
  /// 현재는 네트워크 URL의 경우 썸네일 생성이 지원되지 않습니다.
  Future<void> _generateThumbnailForCache(String videoUrl) async {
    ...
    // 🎯 네트워크 URL의 경우 썸네일 생성은 외부에서 처리해야 함
    // 로컬 파일의 경우 VideoUploadUtils.generateThumbnail을 사용할 수 있지만,
    // 네트워크 URL은 비동기 다운로드가 필요하므로 현재는 지원하지 않음
    debugPrint('[ClipComponent] 썸네일 생성 스킵: 네트워크 URL은 외부에서 처리 필요 - $videoUrl');
  }
  ...
  void _saveAspectRatioToMetadata(double aspectRatio) {
    ...
    try {
      ...
      final existingDouble = (existing is num)
          ? existing.toDouble()
  ...
  double _getAspectRatio(BuildContext context) {
    ...
    try {
      ...
      if (node is ClipNode) {
        final aspectRatioValue = node.metadata['aspectRatio'];
        if (aspectRatioValue != null) {
          final metadataAspectRatio = (aspectRatioValue is num)
              ? aspectRatioValue.toDouble()
              : double.tryParse(aspectRatioValue.toString());
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        ...
        // 리더 모드: 자세히 보기(풀스크린) 버튼
        if (!widget.isEditing &&
            (widget.url.isNotEmpty || widget.localPath.isNotEmpty) &&
            !shouldShowSpinner)
          Positioned(
            top: 8,
            right: 8,
            child: GestureDetector(
              onTap: () {
                final mediaUrl = widget.url.isNotEmpty
                    ? widget.url
                    : 'file://${widget.localPath}';
