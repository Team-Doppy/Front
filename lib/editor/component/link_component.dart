...
import '../utils/link_meta_fetcher.dart';
...
class _LinkComponentState extends State<_LinkComponent>
  ...
  /// 미리보기와 동일: 저장된 썸네일이 없으면 URL 도메인 기준 파비콘 사용
  String _getEffectiveThumbnailUrl() {
    if (widget.thumbnailUrl.isNotEmpty) return widget.thumbnailUrl;
    final raw = widget.url.trim();
    if (raw.isEmpty) return '';
    final normalized = LinkMetaFetcher.normalizeUrl(raw) ?? raw;
    try {
  ...
  Widget _buildThumbnail(BoxConstraints constraints) {
    final displayUrl = _getEffectiveThumbnailUrl();
    if (displayUrl.isEmpty) return _buildIconPlaceholder();

    final decodeWidth = widget.isEditing ? constraints.maxWidth : null;
    ...
    final ImageProvider imageProvider = decodeWidth != null
        ? ResizeImage(baseProvider, width: decodeWidth.toInt())
        : baseProvider;
  ...
  void _showPreview(Offset globalPos) {
    ...
    if (raw.isEmpty) return;
    final String? normalized = LinkMetaFetcher.normalizeUrl(raw);
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          onPageStarted: (String url) {
            debugPrint('[LinkComponent] 웹뷰 로드 시작: $url');
          },
          onPageFinished: (String url) {
            debugPrint('[LinkComponent] 웹뷰 로드 완료: $url');
          },
          onWebResourceError: (WebResourceError error) {
            debugPrint('[LinkComponent] 웹뷰 에러: ${error.description}');
          },
        ),
      )
      ..loadRequest(uri);
  ...
  Widget build(BuildContext context) {
    ...
    // 🎯 읽기 모드: dragService 없을 때 SuperEditorState에서 document 가져오기
    if (doc == null && !widget.isEditing) {
                  ...
                  children: [
                    ...
                    AnimatedSize(
                      ...
                      child: AnimatedSwitcher(
                        ...
                        child: _isCompactViewMode(doc)
                            ? const SizedBox.shrink(
                                key: ValueKey('link_thumb_compact'),
                              )
                            : (_getEffectiveThumbnailUrl().isNotEmpty
                                  ? AspectRatio(
                        ...
                        children: [
                          ...
                          Text(
                            (widget.title.isNotEmpty
                                    ? LinkMetaFetcher.decodeHtmlEntities(
                                        widget.title)
                                    : widget.url),
                            maxLines: 2,
                ...
                children: [
                  GestureDetector(
                    ...
                    onTap: widget.isEditing && widget.dragService != null
                        ...
                        : (!widget.isEditing
                            ? () async {
                                // 🎯 보기 모드: 링크 탭 시 웹브라우저로 열기
                                final raw = widget.url.trim();
                                if (raw.isEmpty) return;
                                final normalized =
                                    LinkMetaFetcher.normalizeUrl(raw) ?? raw;
                                final uri = Uri.tryParse(normalized);
                                if (uri != null &&
