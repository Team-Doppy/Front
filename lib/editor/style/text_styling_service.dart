...
import '../toolbar/toolbar_constants.dart';
...
class TextStylingService extends ChangeNotifier {
  ...
  /// 범위 선택 시 툴바에 보이던 폰트 크기. 범위→collapsed 전환 시 다음 입력에 적용한 뒤 클리어.
  double? _lastFontSizeFromSelection;
  ...
  /// 선택 영역의 ParagraphNode 메타데이터 업데이트 (공통 로직)
  void _updateParagraphMetadataInSelection(
  ...
  void applyFontToMetadata(String fontFamily) {
    ...
    _updateParagraphMetadataInSelection(selection, fontFamily);
  }
  ...
  /// 공통 토글 메서드 (중복 로직 제거)
  void _toggleAttribution(Attribution attribution, {bool saveHistory = false}) {
    final selection = composer.selection;
  ...
  /// 기울임 토글
  void toggleItalic() {
    _toggleAttribution(italicsAttribution, saveHistory: true);
  }

  /// 밑줄 토글
  void toggleUnderline() {
    _toggleAttribution(underlineAttribution, saveHistory: true);
  }

  /// 취소선 토글
  void toggleStrikethrough() {
    _toggleAttribution(strikethroughAttribution, saveHistory: true);
  }
  ...
  void toggleHighlight() {
    ...
    if (hasHighlight) {
      ...
      // 형광펜 적용 (기본 노란색)
      applyHighlight(ToolbarColorPalette.highlightColors.first);
    }
  }

  /// 여러 노드에 걸친 선택 영역에 대한 DocumentSelection 리스트 생성
  List<DocumentSelection> _buildMultiNodeSelections(
    DocumentSelection selection,
  ) {
    ...
    // 여러 노드에 걸친 선택의 경우
    final selections = <DocumentSelection>[];
    ...
    selections.add(
      DocumentSelection(
        ...
        extent: DocumentPosition(
          nodeId: startNode.id,
          nodePosition: TextNodePosition(offset: startNode.text.text.length),
        ),
      ),
    );
  ...
  void applyHighlight(Color color, {DocumentSelection? selectionOverride}) {
    ...
    // 여러 노드에 걸친 선택 영역 처리
    final selections = _buildMultiNodeSelections(selection);
    if (selections.isEmpty) return;

    final requests = selections
        .map(
          (range) => AddTextAttributionsRequest(
            documentRange: range,
    ...
    _removeHighlightAttributions(selectionOverride: selection);
  ...
  void _removeHighlightAttributions({DocumentSelection? selectionOverride}) {
    ...
    for (final range in selections) {
      final startNode = editor.document.getNodeById(range.base.nodeId);
      if (startNode is! TextNode) continue;

      // 해당 범위의 모든 형광펜 속성 찾기
      final highlightAttributions = <Attribution>{};
      final startPos = range.base.nodePosition as TextNodePosition;
      final endPos = range.extent.nodePosition as TextNodePosition;
      final startOffset = startPos.offset;
      final endOffset = endPos.offset;

      for (
        int i = startOffset;
        i < endOffset && i < startNode.text.text.length;
        i++
      ) {
        final attributions = startNode.text.getAllAttributionsAt(i);
        for (final attr in attributions) {
          if (attr is HighlightAttribution) {
            highlightAttributions.add(attr);
          }
        }
      }
  ...
  /// 선택 영역에 등장하는 폰트 크기 집합 (툴바 동기화용, toolbar_utils와 동일 로직)
  Set<double> _getFontSizesInSelection() {
    final selection = composer.selection;
    final sizes = <double>{};
    ...
    if (startNode.id == endNode.id) {
      ...
      for (int i = startIndex; i <= endIndex; i++) {
        final node = editor.document.getNodeAt(i);
        if (node is TextNode) {
          final so = i == startIndex ? startPos.offset : 0;
          final eo = i == endIndex ? endPos.offset : node.text.text.length;
          collectFromNode(node, so, eo);
        }
      }
    }
    return sizes;
  ...
  static List<int> _sampleOffsets(int start, int end) {
  ...
  /// 범위 선택일 때 툴바에 보이는 폰트 크기(단일값)를 저장. collapsed 전환 시 다음 입력에 적용하기 위함.
  void captureFontSizeFromSelection() {
    final selection = composer.selection;
    if (selection == null || selection.isCollapsed) return;
    final sizes = _getFontSizesInSelection();
    if (sizes.isEmpty) {
      _lastFontSizeFromSelection = EditorConfig.defaultBodyFontSize;
    } else if (sizes.length == 1) {
      _lastFontSizeFromSelection = sizes.single;
    } else {
      _lastFontSizeFromSelection = null; // mixed → 적용하지 않음
    }
  }

  /// 선택이 collapsed일 때, 이전 범위 선택에서 저장한 폰트 크기를 preferences/전역에 반영해 툴바와 새 입력이 동기화되도록 함.
  void applyLastSelectionFontSizeWhenCollapsed() {
    final selection = composer.selection;
    if (selection != null && !selection.isCollapsed) return;
    final size = _lastFontSizeFromSelection;
    _lastFontSizeFromSelection = null;
    if (size == null) return;
    if (_globalFontSize != size) {
      _globalFontSize = size;
      notifyListeners();
  ...
  void _removeColorAttributions() {
    ...
    final colorAttributions = existingAttributions
        .where(
          (attr) => attr is ColorAttribution && attr is! HighlightAttribution,
        ) // 형광펜은 제외!
        .toSet();
  ...
  void clearAllStyles() {
    ...
    // 기존 속성들을 모두 수집하여 제거
    final existingAttributions = _getAttributionsInSelection();
    final allAttributions = existingAttributions
        .where(
          (attr) =>
  ...
  void applyFont(FontItem fontItem) {
    ...
    if (selection != null && !selection.isCollapsed) {
      ...
      final fontAttributions = existingAttributions
      ...
      _updateParagraphMetadataInSelection(selection, targetFamily);
    } else {
