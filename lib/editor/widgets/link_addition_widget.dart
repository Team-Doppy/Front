...
class _LinkOverlayState extends State<LinkOverlay>
          ...
          title: Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                filled: true,
                fillColor: theme.onSurface.withOpacity(0.08),
              ...
              Positioned(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    // 🎯 제출 시점에 썸네일 URL 고정 전달 (웹에서 긁어온 이미지가 에디터에 그대로 반영되도록)
            ...
            if (!widget.autoSubmit && _items.isNotEmpty && !_focusNode.hasFocus)
              Positioned(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                    minimumSize: Size(double.infinity, 50),
                    backgroundColor: _items.isEmpty ? null : theme.onSurface,
                    foregroundColor: _items.isEmpty ? null : theme.surface,
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(20),
                    ),
                  ),
  ...
  String? _faviconUrlFor(String rawUrl) {
    final normalized = _normalizeUrl(rawUrl) ?? rawUrl;
    try {
      final uri = Uri.parse(normalized);
      final host = uri.host;
      if (host.isEmpty) return null;
      final domainForIcon = host.replaceFirst('www.', '');
      return 'https://www.google.com/s2/favicons?domain=$domainForIcon&sz=64';
    } catch (_) {
      return null;
    }
  }

  /// 오버레이 미리보기에서 보이는 썸네일(og:image 우선, 없으면 파비콘)을 그대로 전달하기 위한 유틸.
  String? _effectiveThumbnailUrl(String rawUrl, {String? metaThumb}) {
    final t = metaThumb;
    if (t != null && t.isNotEmpty) return t;
    return _faviconUrlFor(rawUrl);
  }
  ...
  void _applyParsedMeta(String html, {_LinkItem? target}) {
    ...
    if (target == null) {
      ...
      if (mounted && !hasMeta && !target.warned) {
        target.warned = true;
        SnackbarUtil.showInfo(context, context.tr('link_may_invalid'));
      }
    }
  }
