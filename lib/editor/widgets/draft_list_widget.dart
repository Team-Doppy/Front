...
class _DraftListWidgetState extends State<DraftListWidget>
  ...
  Future<void> _loadDrafts() async {
    final drafts = await DraftService().getAllDrafts(); // List<DraftData>
    if (mounted) {
      setState(() {
        _drafts = {
          for (final draft in drafts) draft.title: [draft],
        };
        _isLoading = false;
      });
    }
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              title,
                              style: TextStyle(
                                color: isCurrentDraft
                                    ? Theme.of(context).colorScheme.primary
                                    : Theme.of(context).colorScheme.onSurface,
                                fontSize: 16,
                                fontWeight: isCurrentDraft
                                    ? FontWeight.w600
                                    : FontWeight.w500,
                              ),
                              maxLines: 1,
  ...
  String _formatDateTime(DateTime dateTime) {
    return DateFormat('yyyy-MM-dd HH:mm').format(dateTime); // korean time
  }
}
