...
class HintComponentBuilder extends CustomParagraphComponentBuilder {
  const HintComponentBuilder(
    this.hint,
    this.hintStyleBuilder, {
    required super.dragService,
  });
  ...
  Widget? createComponent(
    ...
    return TextWithHintComponent(
      ...
      hintStyleBuilder:
          (attributions) => hintStyleBuilder(componentContext.context),
...
class _ParagraphComponentState extends State<ParagraphComponent>
    ...
    return Column(
      children: [
        ...
        if (_shouldShowTopDropLine())
          Container(height: 3, color: const Color(0xFF007AFF)),
            ...
            children: [
              ...
              Expanded(
                child: TextComponent(
                  ...
                  metadata:
                      widget.viewModel.blockType != null
  ...
  bool _shouldShowTopDropLine() {
    ...
    final shouldShow = dropIndex == currentNodeIndex;
    if (shouldShow) {
      print(
        'Paragraph ${widget.viewModel.nodeId} (인덱스 $currentNodeIndex): 위쪽 라인 표시',
      );
    }
    return shouldShow;
  }
  ...
  bool _shouldShowBottomDropLine() {
    ...
    final shouldShow = dropIndex == currentNodeIndex + 1 && isLastNode;
    if (shouldShow) {
      print(
        'Paragraph ${widget.viewModel.nodeId} (인덱스 $currentNodeIndex): 아래쪽 라인 표시 (마지막 노드)',
      );
    }
    return shouldShow;
  }
  ...
  int _getCurrentNodeIndex() {
    return widget.dragService.getNodeIndex(widget.viewModel.nodeId);
  }
}
...
class DeleteUpstreamAtBeginningOfParagraphCommand extends EditCommand {
  ...
  void execute(EditContext context, CommandExecutor executor) {
    ...
    final componentBefore =
        documentLayoutEditable.documentLayout.getComponentByNodeId(
          nodeBefore.id,
        )!;
