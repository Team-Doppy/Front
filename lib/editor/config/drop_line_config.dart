...
import 'editor_config.dart' as NodeTypeChecker;
  ...
  static ({bool top, bool bottom, bool left, bool right}) resolve({
    ...
    switch (dt.kind) {
      ...
      case DropTargetKind.insertBetweenNodes:
        ...
        // 🎯 이동하는 노드와 이웃한 위치에는 드롭라인 절대 표시 금지
        // - insertIndex == draggingNodeIndex: 자기 노드 바로 위 (원래 자리)
        // - insertIndex == draggingNodeIndex + 1: 자기 노드 바로 아래 (이웃)
        ...
        if (draggingNodeId != null) {
          ...
          if (!isSplitting) {
            ...
            if (draggingNodeIndex != -1) {
              final isAdjacentToDragging = insertIndex == draggingNodeIndex ||
                  insertIndex == draggingNodeIndex + 1;
              if (isAdjacentToDragging) {
