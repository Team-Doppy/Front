...
import 'package:doppy/image/trimmer/properties/trim_editor_painter.dart';
import 'package:doppy/image/trimmer/properties/trim_area_properties.dart';
import 'package:doppy/image/trimmer/properties/trim_editor_properties.dart';
import 'package:doppy/image/trimmer/trimmer.dart' show Trimmer;
import 'package:doppy/image/trimmer/utils/duration_style.dart';
...
class _ScrollableTrimViewerState extends State<ScrollableTrimViewer>
  ...
  void startScrolling(bool isTowardsEnd) {
    _scrollingTimer = Timer.periodic(const Duration(milliseconds: 300), (
      timer,
    ) {
      setState(() {
      ...
      log(
        'SCROLL: $currentScrollValue, (${((_scrollController.position.pixels / _scrollController.position.maxScrollExtent) * 100).toStringAsFixed(2)}%)',
      );
  ...
  void startTimer(bool isTowardsEnd) {
    var start = 300;
    _scrollStartTimer = Timer.periodic(const Duration(milliseconds: 100), (
      Timer timer,
    ) {
      if (start == 0) {
        timer.cancel();
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        ...
        _scrubberAnimation =
            _linearTween.animate(_animationController!)
              ..addListener(() {
                setState(() {});
              })
  ...
  void _onEndDragged() {
    ...
    _videoEndPos =
        (_trimmerAreaDuration * _endFraction) +
        (_scrollController.position.pixels /
                _scrollController.position.maxScrollExtent) *
            _remainingDuration;
    widget.onChangeEnd!(_videoEndPos);
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      Text(
                        Duration(
                          milliseconds: _videoStartPos.toInt(),
                    ...
                    _scrollController.positions.isNotEmpty
                        ? AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            gradient:
                                widget.areaProperties.blurEdges
                                    ? LinearGradient(
