...
import '../../editor/component/clip_component.dart' as ThumbnailVideoPlayer;
import '../../../editor/utils/dialog_util.dart';
import '../../../editor/utils/editor_localization.dart';
import '../editor/trimmer/video_trim_screen.dart';
import 'group_image_layout_screen.dart';
import 'image_editor_screen.dart';
import 'video_editor_screen.dart';
import '../editor/video_specs.dart';
...
class MediaPickerScreen extends StatefulWidget {
  ...
  // ✅ true면 onBeforePop 없이 즉시 pop. 호출자가 result를 받아 처리 (에디터에서 노드 추가/업로드)
  final bool popImmediately;
...
/// 그리드/스트립 썸네일 로딩 시 스피너 대신 표시하는 쉬머 플레이스홀더
Widget _shimmerPlaceholder(BuildContext context) {
  final colorScheme = Theme.of(context).colorScheme;
  final base = colorScheme.onSurface.withOpacity(0.08);
  final highlight = colorScheme.onSurface.withOpacity(0.04);
...
class _MediaPickerScreenState extends State<MediaPickerScreen>
  ...
  static const int _kThumbnailCacheMaxEntries = 120;
  final Map<String, Uint8List> _thumbnailCache = {};
  final List<String> _thumbnailCacheKeys = []; // LRU 순서 유지

  void _putThumbnailCache(String key, Uint8List bytes) {
    if (_thumbnailCache.containsKey(key)) return;
    while (_thumbnailCache.length >= _kThumbnailCacheMaxEntries &&
        _thumbnailCacheKeys.isNotEmpty) {
      final evict = _thumbnailCacheKeys.removeAt(0);
  ...
  Future<bool> _runBeforePop(MediaPickerResult result) async {
    ...
    try {
      ...
      debugPrint('[MediaPicker] onBeforePop stack:\n$st');
  ...
  void initState() {
    ...
    ThumbnailVideoPlayer.pauseAllVideoPlayers(seekToStart: true, mute: true);
    ThumbnailVideoPlayer.muteAllVideos();
    // 🎯 현재 인스턴스 등록
    ...
    if (_mediaType == MediaType.image) {
      ...
      _maxSelectionCount = widget.maxSelectionCount == 1
  ...
  void dispose() {
    ...
    ThumbnailVideoPlayer.cleanupAllVideoPlayers();
    super.dispose();
  }
  ...
  Future<void> _loadMedia() async {
    ...
    try {
      ...
      final filteredAssets = pageAssets.where((asset) {
  ...
  Future<void> _refreshFirstPage() async {
    ...
    try {
      ...
      // ✅ 제한된 접근 상태에서도 타입 필터링 보장 (이미지면 이미지만, 영상이면 영상만)
      final filteredNewPageAssets = newPageAssets.where((asset) {
        if (_mediaType == MediaType.video) {
          return asset.type == AssetType.video;
        } else {
          return asset.type == AssetType.image;
  ...
  Future<void> _loadMoreMedia() async {
    ...
    try {
      ...
      // ✅ 제한된 접근 상태에서도 타입 필터링 보장 (이미지면 이미지만, 영상이면 영상만)
      final filteredNextPage = nextPage.where((asset) {
        if (_mediaType == MediaType.video) {
          return asset.type == AssetType.video;
        } else {
          return asset.type == AssetType.image;
  ...
  Widget _buildSelectedPreviewSection() {
    ...
    return Container(
      ...
      child: selectedAssets.isEmpty
          ? Center(
              child: Text(
                context.tr('media_picker_selected_images_here'),
                style: TextStyle(
          ...
          : ReorderableListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final id = asset.id;
                const stripThumbSize = ThumbnailSize(220, 220);
                final stripCacheKey = '${asset.id}_${stripThumbSize.width}_${stripThumbSize.height}';
                final stripCachedBytes = _thumbnailCache[stripCacheKey];
                return ReorderableDelayedDragStartListener(
                  ...
                  child: Padding(
                    ...
                    child: _SelectedStripItem(
                      ...
                      cachedBytes: stripCachedBytes,
                      cacheKey: stripCacheKey,
                      onThumbnailLoaded: _putThumbnailCache,
                    ),
                  ),
                );
              },
    ...
    try {
      ...
      if (mounted) {
        await DialogUtils.showInfoDialog(
          ...
          message: context.tr('editor_load_failed'),
        );
      }
    } finally {
        ...
        PageRouteBuilder(
          ...
          pageBuilder: (context, animation, secondaryAnimation) {
            return SimpleVideoEditorScreen(
              videoFile: file,
              doneLabelOverride: context.tr('editor_next'), // "추가" 대신 "다음"으로 변경
              onDone: (editorContext, result) async {
                ...
                child: Text(
                  _mediaType == MediaType.video
                      ...
                      ? context.tr(
                            'add_with_count',
                            placeholders: {
                              'count': '${_selectedMediaIds.length}',
                            },
                          )
                  ...
                  style: TextStyle(
                    fontSize: 17,
                    fontWeight: FontWeight.w600,
                    color: _selectedMediaIds.isNotEmpty
            ...
            if (_hasLimitedAccess) {
              ...
              final action = await showCupertinoModalPopup<String>(
                ...
                builder: (context) => CupertinoActionSheet(
                  title: Text(
                    _mediaType == MediaType.video
                        ? context.tr('limited_video_access_title')
                        : context.tr('limited_photo_access_title'),
                    style: TextStyle(
                      fontSize: 13,
                  ...
                  message: Text(
                    _mediaType == MediaType.video
                        ? context.tr('limited_video_access_message')
                        : context.tr('limited_photo_access_message'),
                    style: TextStyle(
                      fontSize: 13,
                      color: colorScheme.onSurface.withOpacity(0.6),
                    ),
                  ...
                  actions: [
                    CupertinoActionSheetAction(
                      ...
                      child: Text(
                        _mediaType == MediaType.video
                            ? context.tr('add_more_video')
                            : context.tr('add_more_photo'),
                        style: TextStyle(
                    ...
                    CupertinoActionSheetAction(
                      ...
                      child: Text(
                        context.tr('allow_full_access_in_settings'),
                        style: TextStyle(
                          color: colorScheme.primary,
                          fontSize: 16,
                        ),
              ...
              if (action == 'add_more') {
                // 추가 사진 선택 피커 열기
                final type = _mediaType == MediaType.image
                    ? RequestType.image
                    : RequestType.video;
                      ...
                      Text(
                        _hasPermission
                            ? (_mediaType == MediaType.video
                                  ? context.tr('limited_video_access_title')
                                  : context.tr('limited_photo_access_title'))
                            : context.tr('photo_library_permission_required'),
                        style: TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.w600,
                          color: colorScheme.onSurface,
                      ...
                      Text(
                        _hasPermission
                            ? (_mediaType == MediaType.video
                                  ? context.tr('limited_video_access_message')
                                  : context.tr('limited_photo_access_message'))
                            : context.tr('open_permission_settings'),
                        style: TextStyle(
                          fontSize: 12,
                          fontWeight: FontWeight.w400,
                ...
                AnimatedSwitcher(
                  ...
                  child: GridView.builder(
                    ...
                    gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                      ...
                      crossAxisSpacing: 1,
                      mainAxisSpacing: 1,
                      childAspectRatio: _mediaType == MediaType.video
                          ? 2 / 3
                          : 4 / 5, // 영상: 2:3, 이미지: 4:5
                    ),
  ...
  Widget _buildMediaThumbnail(AssetEntity asset, int index) {
    ...
    final available =
        (screenWidth - gridPadding - ((_crossAxisCount - 1) * gridSpacing))
    ...
    final cacheKey = '${asset.id}_${thumbSize.width}_${thumbSize.height}';
...
class _VideoThumbnailWidget extends StatelessWidget {
  ...
  final void Function(String key, Uint8List bytes) onThumbnailLoaded;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Stack(
        fit: StackFit.expand,
        children: [
          // ✅ 캐시 있으면 Future 없이 바로 표시 → 선택 토글 시 깜빡임 방지
          RepaintBoundary(
            child: cachedBytes != null
                ? Image.memory(
                    ...
                    errorBuilder: (context, error, stackTrace) {
                      return _videoThumbnailPlaceholder(context);
                    },
                  )
                : FutureBuilder<Uint8List?>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      onThumbnailLoaded(cacheKey, bytes);
                      return Image.memory(
                        bytes,
                        fit: BoxFit.cover,
                        errorBuilder: (context, error, stackTrace) {
  ...
  static Widget _videoThumbnailPlaceholder(BuildContext context) {
    return Container(
...
class _ImageThumbnailWidget extends StatelessWidget {
  ...
  static Widget _imageThumbnailPlaceholder(BuildContext context) {
    return Container(
      color: Theme.of(context).colorScheme.onSurface.withOpacity(0.06),
      child: const Icon(
          ...
          RepaintBoundary(
            child: cachedBytes != null
                ? Image.memory(
                    ...
                    fit: BoxFit.cover,
                    errorBuilder: (context, error, stackTrace) {
                      return _imageThumbnailPlaceholder(context);
                    },
                  )
                : FutureBuilder<Uint8List?>(
                    future: asset.thumbnailDataWithSize(thumbnailSize),
...
class _SelectedStripItem extends StatelessWidget {
  ...
  static Widget _stripPlaceholder(BuildContext context) {
    final colorScheme = Theme.of(context).colorScheme;
    return Container(
              ...
              child: RepaintBoundary(
                child: cachedBytes != null
                    ? Image.memory(
                        ...
                        fit: BoxFit.cover,
                        errorBuilder: (context, error, stackTrace) {
                          return _stripPlaceholder(context);
                        },
                      )
                    : FutureBuilder<Uint8List?>(
