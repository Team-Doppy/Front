...
import '../editor/crop_editor.dart'
    show
        ...
        CropGestureHandler,
        CropAutoZoom;
import '../editor/rotation_editor.dart';
...
class _SimpleImageEditorScreenState extends State<SimpleImageEditorScreen>
  ...
  Future<void> _handleDone() async {
    ...
    try {
      // ✅ 1) 단일 이미지: 편집 화면에서 추가한 경우 개별 이미지로 바로 추가
      if (_images.length == 1) {
      ...
      if (_images.length >= 2) {
        ...
        Map<String, dynamic>? imageDimensions;
        if (layout != GroupImageLayout.individual) {
          ...
          try {
            ...
            for (int i = 0; i < bakedImages.length; i++) {
              final size = await ImageSizeUtils.extractSizeFromBytes(
                bakedImages[i],
                'edited_$i.jpg',
              );
              ...
              // 임시 파일 경로는 나중에 생성되므로, 인덱스 기반으로 저장
              dims['index_$i'] = entry;
            }
            imageDimensions = dims.isNotEmpty ? dims : null;
          } finally {
            if (mounted) {
              // 로딩 오버레이 닫기
                                    ...
                                    child: Center(
                                      child: Row(
                                        ...
                                        children: [
                                          Text(
                                            // 요구사항: "자르기" 대신 "회전" 표기
                                            // (l10n에 rotate/rotation 키가 없어 여기서는 하드코딩)
                                            '회전',
                                            style: TextStyle(
                                              fontSize: 17,
                                              fontWeight: FontWeight.w600,
  ...
  Widget _buildRotatedImage(
    ...
    return RepaintBoundary(
      child: Transform(transform: m, child: imageWidget),
    );
  }
                    ...
                    child: RepaintBoundary(
                      child:
                          ...
                          ? (() {
                              ...
                              final cs =
                                  (_isClosingAfterCropApply &&
                                      _closingCropContainerSizes[index] != null)
                      ...
                      builder: (context, _) {
                        ...
                        return Opacity(
                          ...
                          child: CropHandleBuilder.buildCropHandles(
                            ...
                            screenImageRect:
                                imageRectForCrop, // ✅ freeze된 imageRect 사용
                            imageSize: imageSize,
                            cropRectScreen:
                                cropRectScreen, // ✅ 항상 계산된 값 전달 (freeze 금지)
                          ),
                        );
                      },
                    ),
