...
import '../editor/crop_editor.dart'
    show
        CropState,
        CropUtils,
        ...
        CropDragEndResult,
        AutoZoomResult;
import '../editor/adjustment_editor.dart';
import '../editor/filter_editor.dart';
import '../editor/speed_editor.dart';
import '../utils/filter_presets.dart';

/// 간단한 커스텀 비디오 편집 화면
/// 바텀시트 기반 UI, 실시간 미리보기 제공
class SimpleVideoEditorScreen extends StatefulWidget {
  SimpleVideoEditorScreen({
    super.key,
    this.videoFile,
    this.videoFileList,
...
class _SimpleVideoEditorScreenState extends State<SimpleVideoEditorScreen>
  ...
  Widget _buildBottomSheetEditorContent() {
    final content = _editMode == _EditMode.crop
                            ...
                            child: Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  if (!_isBottomSheetOpen)
                                    GestureDetector(
                                      onTap: (_isExporting || _isPageScrolling)
                                          ? null
                                          : _handleDone,
                                      child: ClipRRect(
  ...
  Widget _buildRotatedVideo(
    ...
    final widgetKey = _editMode == _EditMode.adjust
    ...
    final finalDisplayW = displayW > 0
        ? displayW
    ...
    if (_getColorFilter(state) != null) {
      videoWidget = ColorFiltered(
        key: _editMode == _EditMode.adjust
    ...
    final anchor = (isCropApplied && currentImageRect != null)
        ...
        return AnimatedBuilder(
          ...
          builder: (context, _) {
            ...
            final preview = GestureDetector(
              ...
              onScaleStart: _editMode == _EditMode.crop
                  ...
                  : null,
              onScaleUpdate: _editMode == _EditMode.crop
                  ...
                  : null,
              onScaleEnd: _editMode == _EditMode.crop
                  ? (details) =>
                        _onCropScaleEnd(details, index, effectiveContainerSize)
                  : null,
              // 필터 모드는 기존 Pan 유지
              onPanUpdate: _editMode == _EditMode.filter
                  ? _onFilterSwipe
                  : null,
              onPanEnd: _editMode == _EditMode.filter
                  ? _onFilterSwipeEnd
                  : null,
              child: Stack(
                children: [
                      ...
                      builder: (context) {
                        ...
                        if (controller == null ||
                          ...
                          return Positioned(
                            ...
                            child: Center(
                              child: widget.isExistingNodeEdit
    ...
    if (_editMode == _EditMode.crop) {
      ...
      if (containerSize != null &&
        ...
        if (state.cropState.isCropRectInitialized &&
          ...
          final targetAspectRatio = state.cropState.selectedAspectRatio != null
              ? CropGestureUtils.parseAspectRatio(
                  state.cropState.selectedAspectRatio,
                )
              : null;
  ...
  Widget _buildCropBottomSheet() {
    ...
    return CropEditorBottomSheet(
      ...
      onRotationChanged: (v) => setState(() {
        // ✅ ±45도로 제한
        state.rotation = v.clamp(-45, 45);
      }),

      onResetRotation: () => setState(() {
        state.rotation = 0;
        state.rotationQuarterTurns = 0;
      }),
      onToggleFlipHorizontal: () => setState(() {
        state.flipHorizontal = !state.flipHorizontal;
      }),
      onToggleFlipVertical: () => setState(() {
  ...
  Widget _buildFilterBottomSheet() {
    ...
    final thumbnailFuture = cachedThumbnail != null
  ...
  Future<Uint8List?> _generateVideoThumbnail(File videoFile) async {
    try {
      ...
      final thumbnailGenerator = widget.thumbnailGenerator;
      if (thumbnailGenerator != null) {
        final thumbnailBytes = await thumbnailGenerator(
          videoFile.path,
          0, // 첫 프레임 (timeMs)
          75, // quality
        );
        ...
        debugPrint(
          '[SimpleVideoEditor] ⚠️ thumbnailGenerator가 제공되지 않아 썸네일 생성을 스킵합니다.',
        );
        return null;
      }
    } catch (e) {
  ...
  Widget _buildAdjustmentBottomSheet() {
    ...
    // AdjustmentState로 변환
    final adjustmentState = AdjustmentState()
      ..brightness = state.brightness
      ..contrast = state.contrast
      ..saturation = state.saturation
      ..luminance = state.luminance
