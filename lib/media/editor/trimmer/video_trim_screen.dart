...
class Trimmer extends ChangeNotifier {
  ...
  /// Creates a [Trimmer] instance.
  ///
  /// - [thumbnailGenerator] is an optional callback function that generates thumbnail bytes.
  ///   If not provided, thumbnail generation will return null.
  Trimmer({this.thumbnailGenerator});
    ...
    _playbackTimer = Timer.periodic(const Duration(milliseconds: 33), (timer) {
      ...
      if (isPlaying) {
        ...
        if (position >= stopThreshold) {
          ...
          final startMs = ((_startValue.clamp(0.0, double.infinity)) * 1000)
              .toInt();
  ...
  Future<void> seekTo(double position, {bool clampToRange = true}) async {
    ...
    // 🎯 클램프 적용
    final clampedPosition = clampToRange
        ? position.clamp(
            _startValue.clamp(0.0, double.infinity),
            _endValue.clamp(0.0, double.infinity),
          )
        : position.clamp(0.0, maxSeconds > 0 ? maxSeconds : double.infinity);
    ...
    try {
      ...
      for (int i = 0; i < frameCount; i++) {
        ...
        if (thumbnailGenerator != null) {
          thumbnailGenerator!(_videoFile!.path, timeMs, 70)
              .then((bytes) {
          ...
          debugPrint('[Trimmer] ⚠️ thumbnailGenerator가 제공되지 않아 썸네일 생성을 스킵합니다.');
        }
      }
...
class VideoViewer extends StatelessWidget {
  ...
  const VideoViewer({
    ...
    this.imageToScreenRect,
    this.coverScaleToContainCropRect,
    this.getAdjustmentMatrix,
    this.blurSigmaForPreview,
  });
  ...
  ({Rect imageRect, Rect cropRectScreen}) _computeCropLayout({
    ...
    if (computeImageRect == null || imageToScreenRect == null) {
      // Fallback: return basic rects if callbacks not provided
      ...
      return (imageRect: baseRect, cropRectScreen: cropRectImage);
    }

    // 1) scale=1, offset=0 기준 rect
    final baseRect = computeImageRect!(
    ...
    final baseCropScreen = imageToScreenRect!(
    ...
    final scaledRect = computeImageRect!(
      containerSize: containerSize,
      imageSize: videoSize,
      scale: scaleFactor,
      offset: Offset.zero,
    ...
    final scaledCropScreen = imageToScreenRect!(
      imageRect: cropRectImage,
    ...
    final finalRect = computeImageRect!(
      containerSize: containerSize,
      imageSize: videoSize,
      scale: scaleFactor,
      offset: offsetDelta,
    ...
    final finalCropScreen = imageToScreenRect!(
      imageRect: cropRectImage,
  ...
  ColorFilter? _getColorFilterFromSpec(VideoEditSpec? spec) {
    ...
    if (getAdjustmentMatrix == null) return null;

    // Note: Filter support requires external FilterModel and FilterUtils
    // For now, only adjustment matrix is supported via callback
    final adjustmentMatrix = getAdjustmentMatrix!(
      ...
      builder: (context, _) {
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            ...
            final fullImageRect = computeImageRect != null
                ? computeImageRect!(
                    containerSize: containerSize,
                    imageSize: videoSize,
                    scale: 1.0,
            ...
            if (spec?.cropRectImage != null &&
                computeImageRect != null &&
                imageToScreenRect != null) {
            ...
            final sigma = blurSigmaForPreview != null
                ? blurSigmaForPreview!(
                    blur: (spec?.blur ?? 0.0),
            ...
            double kCover = 1.0;
            if (cropRectScreen != null && coverScaleToContainCropRect != null) {
              kCover = coverScaleToContainCropRect!(
  ...
  Widget _buildTimeline() {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // 🎯 썸네일 스트립 너비 계산
        final thumbnailStripWidth = totalSeconds <= maxTrimLength
        ...
        return Stack(
          ...
          children: [
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Builder(
                          builder: (context) {
                            ...
                            // 🎯 실제 청크 너비 (최소 너비 보장)
                            final actualChunkWidth =
                                (thumbnailStripWidth / effectiveChunkCount)
            ...
            Positioned(
              ...
              top: -2,
              bottom: -2,
              width: endBoundaryX > totalWidth - handleDragAreaWidth / 2
                  ? (totalWidth -
                        math.min(
                          endBoundaryX - handleDragAreaWidth / 2,
                          totalWidth - handleDragAreaWidth,
        ...
        return Positioned(
          ...
          child: GestureDetector(
            ...
            onPanStart: (details) {
              ...
              final startScrollOffset = _scrollController.hasClients
                  ? _scrollController.offset
                  : 0.0;
  ...
  Widget _buildDraggableOverlay({
    ...
    return GestureDetector(
      ...
      onPanStart: (details) {
        ...
        setState(() {
          ...
          if (showOverlay) {
            ...
            _overlayDragStartScrollOffset = _scrollController.hasClients
                ? _scrollController.offset
                : 0.0;
            ...
            _dragStartScrollOffset = _scrollController.hasClients
                ? _scrollController.offset
                : 0.0;
            _dragStartValue = isStart
                ? widget.trimmer.startValue
                : widget.trimmer.endValue;
            _dragStartOppositeValue = isStart
                ? widget.trimmer.endValue
                : widget.trimmer.startValue;
      ...
      onPanUpdate: (details) {
        ...
        final currentOppositeValue = isStart
            ? widget.trimmer.endValue
            : widget.trimmer.startValue;
            ...
            children: [
              ...
              ValueListenableBuilder<double>(
                ...
                builder: (context, currentPos, _) {
                  ...
                  final effectiveRelative = speed > 0
                      ? (rawRelative / speed)
                      : rawRelative;
...
class VideoTrimScreen extends StatefulWidget {
  ...
  showInfoDialog;
  final String Function(BuildContext context, String key)? translate;
  ...
  // Callback for Trimmer thumbnail generation
  final Future<Uint8List?> Function(
    String videoPath,
    int timestamp,
    int quality,
...
class _VideoTrimScreenState extends State<VideoTrimScreen> {
  ...
  void initState() {
    super.initState();
    _trimmer = Trimmer(thumbnailGenerator: widget.thumbnailGenerator);
    _initializeTrimmer();
  }
  ...
  Future<void> _initializeTrimmer() async {
    try {
      ...
      if (widget.showInfoDialog != null && widget.translate != null) {
        await widget.showInfoDialog!(
          context,
          title: widget.translate!(context, 'error'),
  ...
  Future<void> _trimVideo() async {
    ...
    try {
      ...
      if (widget.generateThumbnail != null) {
        try {
          final thumb = await widget.generateThumbnail!(
      ...
      if (widget.showInfoDialog != null && widget.translate != null) {
        await widget.showInfoDialog!(
          context,
          title: widget.translate!(context, 'error'),
          message: widget.translate!(context, 'video_trim_failed'),
        );
      }
    } finally {
      if (!mounted) return;
          ...
          child: Text(
            widget.fromEditor
                ...
                : (widget.translate != null
                      ? widget.translate!(context, 'cancel')
                      : '취소'),
            style: TextStyle(
              fontSize: 17,
              fontWeight: FontWeight.w600,
              ...
              children: [
                Expanded(
                  child: VideoViewer(
                    trimmer: _trimmer,
                    editSpec: widget.editSpec,
                    computeImageRect: widget.computeImageRect,
                    imageToScreenRect: widget.imageToScreenRect,
                    coverScaleToContainCropRect:
                        widget.coverScaleToContainCropRect,
                    getAdjustmentMatrix: widget.getAdjustmentMatrix,
                    blurSigmaForPreview: widget.blurSigmaForPreview,
                  ),
                ),
                TrimEditor(
                  trimmer: _trimmer,
