...
class Trimmer {
    ...
    for (
      ...
      try {
        ...
        if (thumbnailGenerator != null) {
          thumbnail = await thumbnailGenerator!(videoPath, timeMs, qualityGIF);
        }
  ...
  Future<void> saveTrimmedVideo({
    ...
    if (outputType == OutputType.gif) {
      ...
      // TODO: 비디오 트리밍 구현
      // 프로젝트에서는 FFmpeg를 사용하므로, 여기서는 경로만 반환하거나
      // 실제 트리밍은 호출하는 쪽에서 처리하도록 할 수 있습니다.
      //
      // 예시: FFmpeg를 사용한 트리밍
      // final duration = endValue - startValue;
      // final command = '-ss ${startValue.toStringAsFixed(2)} '
      //     '-i "${currentVideoFile!.path}" '
