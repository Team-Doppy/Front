...
class FriendService {
  ...
  Future<void> sendFriendRequest(String targetUsername) async {
    try {
      debugPrint('🚀 [FriendService] 친구 요청 보내기 시작: $targetUsername');
      ...
      debugPrint('✅ [FriendService] 응답 받음 | code=$code data=${response.data}');

      // 일부 서버는 201/204를 반환할 수 있음 → 2xx 모두 성공 처리
      if (code != null && code >= 200 && code < 300) {
        debugPrint('✅ [FriendService] 친구 요청 성공: $targetUsername');
        return; // 성공
      }

      debugPrint('⚠️ [FriendService] 예상치 못한 응답 코드: $code');
      throw Exception('친구 신청 실패: 응답 코드 $code');
    } catch (e) {
      debugPrint('❌ [FriendService] 에러 발생: $e');

      if (e is DioException) {
        final code = e.response?.statusCode;
        final responseData = e.response?.data;

        debugPrint('❌ [FriendService] DioException | code=$code');
        debugPrint('❌ [FriendService] responseData: $responseData');
        debugPrint('❌ [FriendService] error message: ${e.message}');

        // 🎯 특수 케이스: 이미 친구 요청이 존재하는 경우 → 성공으로 간주
        if (code == 400 &&
            responseData.toString().contains('이미 친구 요청이 존재합니다')) {
          debugPrint('✅ [FriendService] 이미 친구 요청이 존재함 → 성공 처리');
          return; // 성공으로 처리
        }
        ...
        if (code == 400 &&
          ...
          try {
            debugPrint('🔄 [FriendService] fallback 시도: 쿼리파라미터 방식');
            ...
            if (fcode != null && fcode >= 200 && fcode < 300) {
              debugPrint('✅ [FriendService] fallback 성공!');
              return;
            }
            // ignore: avoid_print
            debugPrint(
            ...
            // ignore: avoid_print
            debugPrint(
              '[FriendService] fallback request failed: $fallbackError',
            );
          }
        }
      }
  ...
  Future<void> deleteFriendsBatch(List<String> usernames) async {
    try {
      debugPrint('🔍 [FriendService] 친구 일괄 삭제 시작: ${usernames.length}명');
      ...
      debugPrint('📡 [FriendService] API 응답 상태: ${response.statusCode}');
      debugPrint('📡 [FriendService] API 응답 데이터: ${response.data}');

      if (response.statusCode == 200) {
        debugPrint('✅ [FriendService] 친구 일괄 삭제 성공');
        return;
      } else {
        debugPrint(
      ...
      debugPrint('❌ [FriendService] 친구 일괄 삭제 중 예외 발생: $e');
      if (e is DioException) {
        debugPrint(
  ...
  Future<void> blockUser(String targetUsername) async {
    try {
      ...
      if (response.statusCode != null &&
        ...
        debugPrint('[FriendService] 사용자 차단 성공: $targetUsername');
        return;
      }
      ...
      if (e is DioException) {
        ...
        debugPrint('[FriendService] 사용자 차단 실패: $errorMessage');
  ...
  Future<void> unblockUser(String targetUsername) async {
    try {
      ...
      if (response.statusCode != null &&
          response.statusCode! >= 200 &&
          response.statusCode! < 300) {
        debugPrint('[FriendService] 차단 해제 성공: $targetUsername');
        return;
      }
      ...
      if (e is DioException) {
        ...
        debugPrint('[FriendService] 차단 해제 실패: $errorMessage');
        throw Exception(errorMessage);
      }
      rethrow;
    }
  ...
  Future<String> unblockUsersBatch(List<String> usernames) async {
    try {
      ...
      if (response.statusCode != null &&
        ...
        debugPrint('[FriendService] 다중 차단 해제 성공: $message');
        return message;
      }
      ...
      if (e is DioException) {
        ...
        debugPrint('[FriendService] 다중 차단 해제 실패: $errorMessage');
        throw Exception(errorMessage);
      }
      rethrow;
    }
  ...
  Future<void> cancelFriendRequest(String targetUsername) async {
    try {
      ...
      debugPrint('┌─[RES] ${response.statusCode} ${response.data}');
      debugPrint('└────────────────────────────────');
      if (response.statusCode != 200) {
